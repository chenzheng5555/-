...
相比于0-1的一次放入，完全背包可以重复放入。处理第$i$个时，1）如果不放，则取$f_{i-1,j}$；2）如果放，因为可以重复放，分为第一次放$f_{i-1,j-w_i}+v_i$和重复放$f_{i,j-w_i}+v_i$，当$j^*=j-w_i$时（上一次取第$i$个的情况），由于$f_{i,j^*}$已经考虑了$f_{i-1,j^*}$（第一项）
$$
...
长度为$n$的字符串$s$，前缀函数为长度为$n$的数组$\pi$，其中$\pi[i]$表示：子串$s[0...i]$的相等的真前缀$s[0...k-1]$和真后缀$s[i-(k-1)...i]$ 对的最大长度$k$。
