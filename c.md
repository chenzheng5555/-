...
在编译时要加上`-g`选项，生成的目标文件才能用gdb进行调试；其作用为，**在目标文件中加入源代码的信息**。在调试时目标文件时必须**保证gdb也能找到源文件**。
...
1. **gcc编译产生的目标文件不会有_start**，整个程序的入口点是**crt1.o**中提供的**_start**。它首先做一些初始化工作（Startup Routine），然后**调用C代码中提供的main函数**。用gcc链接目标文件时，gcc其实是调用`ld`将目标文件crt1.o和我们的hello.o链接在一起。相当于`exit(main(argc, argv))`。
