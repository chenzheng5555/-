...
类似的题还有：[154. 寻找旋转排序数组中的最小值 II](https://leetcode-cn.com/problems/find-minimum-in-rotated-sorted-array-ii/)、[153. 寻找旋转排序数组中的最小值](https://leetcode-cn.com/problems/find-minimum-in-rotated-sorted-array/)、[33. 搜索旋转排序数组](https://leetcode-cn.com/problems/search-in-rotated-sorted-array/)、
...
class Solution {
    int n;
    //p表示与前一个相等的个数，q表示深度，用来计算排列数，q!为总的排列数,p!为重复排列数。y=q!/p!为满足要求的排列数
    int dfs(vector<int>& nums,int k,int t,long p,long q,long long y){
        //printf("%d,%d,%d,%d,%d,%d\n",k,t,p,x,q,y);
        if(t<0)return 0;
        if(t==0)return y;
        int ans=0;
        for(int i=k;i<n;++i){
            if(i!=k)p=0
            ans+=dfs(nums,i,t-nums[i],p+1,q+1,(y/(p+1))*(q+1));
        }
        return ans;
    }
...
public:
    int combinationSum4(vector<int>& nums, int target) {
        n=nums.size();
        int ans=dfs(nums,0,target,0,0,1);
        return ans;
    }
};
```
...
```c++
//统计字符串里每个字符的出现次数，如果遇到次数小于k的，则答案只可能在两边，递归调用
//还分别求由1，2...26个字符各自组成的满足要求的结果，然后取最大值
class Solution {
	...
	int dfs(string s, int k) {
		...
		for (auto c : s) {
			int id = c - 'a';
			cnt[id]++;
			if (cnt[id] == 1)num++, total++;
			if (cnt[id] == k)num--;
		}
		if (num == 0)return n;//出现次数都大于k
		if (num == total)return 0;//没有字符出现次数大于k
		int pre_i = -1, ans = 0;
		for (int i = 0; i < n; ++i) {
			int id = s[i] - 'a';
			if (cnt[id] > 0 && cnt[id] < k) {
				int max_ans = dfs(s.substr(pre_i + 1, i - pre_i - 1), k);
				ans = max(ans, max_ans);
				pre_i = i;
			}
		}
		if (pre_i < n - 1) {
			int max_ans = dfs(s.substr(pre_i + 1, n - pre_i - 1), k);
			ans = max(ans, max_ans);
		}
		return ans;
	}
...
```c++
//说相等的数可以属于一种颜色，说相同数的兔子数量超过该数后，则为不同颜色。
class Solution {
public:
	int numRabbits(vector<int>& answers) {
		int cnt = 0;
		//unordered_map<int, int>maps;//题目规定了最大的n,使用数组作为哈希更快
		int maps[1000] = { 0 };
		for (auto n : answers) {
...
```c++
/*二叉树的中序遍历
 * 因为搜索二叉树的中序遍历得到的为递增序列，即可设置一个变量存储上一次访问的节点。
 */
class Solution {
...
public:
	...
	int clumsy(int N) {
		...
		int k = N / 4, m = N % 4;
		int y = N + 1 + 2 / (N - 2);
		//long sum = k * (N - 3) - 2 * (k - 1) * k;
		//long dif = y - (k - 1) * y + 2 * (k - 1) * k;
		long pre = (long)k * (N - 3) + y - (k - 1) * y;
		if (m == 0 && k > 1)pre -= 1;
		else if (m < 3)pre -= m;
		else pre -= 6;
		return pre;
...
```c++
//规律，还可以通过二分查找，找出最h=2*r+1,[1,min(r),(r+1)^2,],[max(r),n/2,1],reamin/n
class Solution {
public:
	int maxValue(int n, int index, int maxSum) {
		int l = maxSum - n, ans = 1;
		int left = index, right = index;
		while (l) {
			l -= (right - left + 1);
			if (l >= 0)ans++;
			else break;
			if (right == n - 1 && left == 0) {//此后一次增加，会减少n
				int h = l / n;
				ans += h;
				break;
			}
...
public:
	bool areSentencesSimilar(string sentence1, string sentence2) {
		...
		if (i == 0) {//句子1与句子2的头不匹配，则后面一定相等
			...
			if (sentence2[j] != ' ')return false;//句子2最后一个单词不与句子1的最后一个单词匹配
		}
		else {
			if (sentence1[i - 1] != ' ')return false;//匹配的前半部分中的最后一个单词不匹配
			n--, m--;
			while (n >= i && sentence1[n] == sentence2[m])n--, m--;
			if (n > i)return false;//句子1的后半部分无法与句子匹配
		}
		return true;
	}
};
...
#### [5708. 统计一个数组中好对子的数目](https://leetcode-cn.com/problems/count-nice-pairs-in-an-array/)
...
public:
	int countNicePairs(vector<int>& nums) {
		unordered_map<int, int>maps, offmap;//使用multiset会超时
		int cnt = 0;
		for (auto n : nums) {
			if (maps.find(n) == maps.end()) {
				maps[n] = rev(n);
			...
			int off = n - maps[n];
			if (offmap.find(off) == offmap.end()) {
				offmap[off] = 0;
			}
			cnt = (cnt + offmap[off]) % mod;
			offmap[off]++;
		}
		return cnt;
	}
...
public:
	...
	int minAbsoluteSumDiff(vector<int>& nums1, vector<int>& nums2) {
		...
		for (int i = 0; i < n; ++i) {
			st.insert(nums1[i]);
			all = (all + abs(nums1[i] - nums2[i])) % mod;
		}
		int maxn = 0; //交换后减少的最大值。
		for (int i = 0; i < n; ++i) {
			auto it = st.lower_bound(nums2[i]);
			int cur = abs(nums1[i] - nums2[i]);
			if (it != st.end()) {
				maxn = max(maxn, cur - abs(*it - nums2[i]));//最靠近且大于nums2[i]的值
			}
			if (it != st.begin()) {
				maxn = max(maxn, cur - abs(*(--it) - nums2[i]));//最靠近且小于nums2[i]的值
			}
		}
		all -= maxn;
		return all >= 0 ? all % mod : all + mod;
	}
};
```
...
public:
	int findTheWinner(int n, int k) {
		vector<int>next(n + 1, 1);//建立链表
		for (int i = 1; i < n; ++i) {
			next[i] = i + 1;
		}
		int start = 1, off, pre = n;
		while (n > 1) {//剩余人数大于1
			off = (k - 1) % n;//省略掉整圈数，
			while (off-- > 0) {//移动
				pre = start;
...
class Solution {
	vector<vector<int>>mem;
	int dfs(vector<int>& o, int k, int side, int n) {
		if (mem[side - 1][k] != -1)return mem[side - 1][k];
		int s = k;
		while (k < n && (o[k] == 0 || o[k] != side))k++;

		int minC = INT_MAX;
		if (k != n) {//未达到终点
			k--;//障碍前的位置
			for (int i = 1; i < 4; ++i) {
				if (i == side || o[k] == i)continue;//跳到的赛道有障碍
				int cnt = dfs(o, k + 1, i, n) + 1;//跳到另一个赛道 位置加一与否都可以
				minC = min(minC, cnt);
			}
		}
		else minC = 0;
		for (int i = s; i < k; ++i) mem[side - 1][i] = minC;//存储该通道到终点的最小侧跳次数
...
```c++
/* 操作次数为目的数与原数的差，只能将小的数增加为大的数。所以先排序，第i个数的频数最大为i
 * 将前面的数变为目的数nums[i]，需要的操作数为x=nums[i]*(i-j)-(sum[i-1]-sum[j-1]),只要k>=x即可
 * 因为x是单调递减的。所以可以使用二分法。
 */
class Solution {
public:
    int maxFrequency(vector<int>& nums, int k) {
        ...
        pre[0] = nums[0];
        for (int i = 1; i < n; ++i) {
            int r = i - 1, l = 0, j = i; //j表示最小的j,使得x<=i
            while (l <= r) {
                int m = l + (r - l) / 2, p = 0;
                if (m)p = pre[m - 1];
                int x = (long long)(i - m) * nums[i] - pre[i - 1] + p;
                if (x < k)r = m - 1, j = m;
                else if (x > k)l = m + 1;
                else {
                    j = m;
...
```c++
/* 状态机，根据相邻两个字符的确定状态的转化，
 * 直接比较也ok，先找到初始状态，再继续状态转移，无法转移，重新找初始状态。
 */
class Solution {
public:
    int longestBeautifulSubstring(string word) {
        int n = word.size();
        unordered_map<char, int>mp{ {'a',1},{'e',2},{'i',3},{'o',4},{'u',5} };//状态机
        int f = -1, i,j = 0, ans = 0;
        for (i = 0; i < n; ++i) {
            if (mp[word[i]] != f) {//状态变化
                //cout << i << ",";
                if (mp[word[i]] == f + 1) {//下状态
                    f++;
                }
                else {//出错
                    if (f == 5)ans = max(ans, i - j);//满足要求的一个解
                    j = i;
                    if (word[i] == 'a')f = 1;
                    else f = -1, j++;
                }
...
```c++
//超时，思路是如果当前超出了，往回修改。n*n
class Solution {
public:
    int maxBuilding(int n, vector<vector<int>>& restrictions) {
        vector<int>h(n, 0), r(n, INT_MAX);
        int ans = 0;
...
/* 每个位置的高度除了自己的限制，还受来自左右两边的限制，取较小的那个。
 * 可以从左到右、从右到左，将左右的限制加在每个限制上。使其为真正的限制。
 * 则两个相邻限制中间的最大高度为h1+x=h2+j-i-x=h,所以h=(h1+h2+j-i)/2
 */
class Solution {
public:
    int maxBuilding(int n, vector<vector<int>>& restrictions) {
        ...
        int preh = 0, id = 1;//上一个高度和下标
        for (int i = 0; i < m; ++i) {//从左往右
            int j = restrictions[i][0], h = restrictions[i][1];
            restrictions[i][1] = min(h, j - id + preh);
            id = j, preh = restrictions[i][1];
        }
        // for(auto x:restrictions)cout<<"],["<<x[0]<<","<<x[1];
        // cout<<endl;
        preh = restrictions[m - 1][1], id = restrictions[m - 1][0];
        int ans = n - id + preh;
...
```c++
//(3,6)(7,4)(6,4)(1,5)(4,4)(3,4)(7,2)(8*3)(1,3)(2,3)(8,3)(5,2)(9,1)(8,1)(3,2)(2,2)(7,1)(5,1)(1,2)(1,1)
//(3,7)(7,5)(6,5)(1,6)(4,5)(3,5)(7,2)(8,3)(1,4)(2,3)(8,3)(5,2)(9,1)(8,1)(3,2)(2,2)(7,1)(5,1)(1,2)(1,1)
class Solution {
public:
...
public:
	int storeWater(vector<int>& bucket, vector<int>& vat) {
		int n = vat.size(), ans = 0;
		need.assign(n, 0);
		priority_queue<int, vector<int>, cmp>que;//需要倒水次数 最大的下标
		for (int i = 0; i < n; ++i) {
			if (bucket[i] == 0 && vat[i])ans++, bucket[i]++;
			if (bucket[i] == 0 || vat[i] == 0)continue;
			need[i] = (vat[i] + bucket[i] - 1) / bucket[i];
			que.push(i);
		}
		if (que.empty())return ans;

		int maxCnt = need[que.top()];
		int minAns = ans + maxCnt;
		while (maxCnt > 1) {
			maxCnt = need[que.top()];
			//cout<<maxCnt<<","<<minAns<<"),";
			int cnt = 0, j;
			...
			int m = que.empty() ? M : max(M, need[que.top()]);
			m = min(m + cnt, maxCnt);
			minAns = min(minAns, ans + m);//
			for (auto x : index) {
				que.push(x);
			}
			ans += cnt;
...
public:
    int storeWater(vector<int>& b, vector<int>& v) {
        ...
        int ans = 100000000;
        for (int i = 1; i <= 10000; ++i){//倒水次数
            int s = i;
            for (int j = 0; j < n; ++j)
                s += max(0, (v[j] + i - 1) / i - b[j]);//倒水次数为i时需要的最小高度、减去本来的高度，为扩容次数。
            ans = min(ans, s);
        }
        return ans;
    }
...
//leetcode快排的方法
class Solution {
	int partition(vector<int>& nums, int l, int r) { //还有一种方法是左右两个指针进行比较
		int pivot = nums[r];
		int i = l - 1;
		for (int j = l; j <= r - 1; ++j) {
			if (nums[j] <= pivot) { // i+1位置为值比pivot大的，目前却找到一个比povit小的数，所以交换两者。
				i = i + 1;
				swap(nums[i], nums[j]);
			}
		}
		swap(nums[i + 1], nums[r]);//povit该去的位置
		return i + 1;
	}
	// 基于随机的划分
	int randomized_partition(vector<int>& nums, int l, int r) {
		int i = rand() % (r - l + 1) + l;
		swap(nums[r], nums[i]); // 随机找一个povit数
	...
	void randomized_selected(vector<int>& arr, int l, int r, int k) {
		...
		else if (k < num) {//k在povit的右边，
			randomized_selected(arr, l, pos - 1, k);
		}
		else { //在povit的左边,还差k-num个数构成最小k个数
	...
	vector<int> findSquare(vector<vector<int>>& matrix) {
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < m; ++j) {
				if (matrix[i][j] == 0) {
					int c = j, r = i;
					while (c < m && r < n && matrix[i][c] == 0 && matrix[r][j] == 0) {
						c++, r++;
					}
					int c1 = j, r1 = i;
					c--, r--;
					while (c >= j && (ans.empty() || ans[2] <= c - j)) {//可能出现更大的结果
						while (c1 <= c && r1 <= r && matrix[r][c1] == 0 && matrix[r1][c] == 0) {
							r1++, c1++;
						}
						if (c1 - j == c - j + 1 && (ans.empty() || ans[2] < c1 - j)) {
							ans = { i,j,c1 - j };
							break;
						}
						c--, r--, c1 = j, r1 = i;//缩短长度
